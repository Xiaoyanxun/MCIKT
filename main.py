...
class DataReader(object):
    ...
    def __init__(self, args):
        ...
        self.inter_df = pd.read_csv(os.path.join(self.prefix, self.dataset, 'interactions.csv'), sep=self.sep ,encoding='UTF-8')

        # aggregate by user
        user_wise_dict = dict()
        cnt, n_inters = 0, 0
        for user, user_df in self.inter_df.groupby('user_id'):
            df = user_df[:self.max_step]  # 考虑前50条
            user_wise_dict[cnt] = {
                'user_id': user,
                ...
                'dwell_seq': df['dwell_time'].values.tolist(),
                'interval_seq': df['interval'].values.tolist()
            }
            cnt += 1
            n_inters += len(df)
        ...
        self.n_stamps = max(self.inter_df['timestamp']) + 1
        self.n_dwells = max(self.inter_df['dwell_time']) + 1
        self.n_interval = max(self.inter_df['interval']) + 1
        
        
        logging.info('"n_users": {}, "n_skills": {}, "n_problems": {}, "n_stamps": {}, "n_dwells": {}, "n_interactions": {}'.format(
            self.n_users, self.n_skills, self.n_problems, self.n_stamps, self.n_dwells, n_inters
        ))
    ...
    def gen_fold_data(self, k):
        ...
        dev_size = int(0.1 * len(residual_df))    #验证集10%
